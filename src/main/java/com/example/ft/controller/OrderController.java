...
public class OrderController {
    ...
    // 주문 생성
    @PostMapping // HTTP POST 요청을 처리합니다. 클라이언트가 POST 요청을 보내면 이 메서드가 호출
    public ResponseEntity<String> createOrder(@RequestBody Order order) { // 클라이언트가 전송한 JSON 데이터를 Order 객체로 변환하여 받습니다.
        try {
            orderService.insertOrder(order); // 받은 주문 정보를 서비스 계층의 insertOrder 메서드에 전달하여 데이터베이스에 주문을 저장
            
            // ResponseEntity: 응답을 보낼 때 사용하는 객체입니다. 여기서는 생성된 주문에 대한 응답을 보냅니다.
            // HttpStatus.CREATED: 주문이 성공적으로 생성되었음을 나타내는 상태 코드
